...
def send_reset_email(email: str, name: str, reset_link: str) -> bool:
    """Envia e-mail de recuperação de senha via SMTP2GO HTTP API."""
    ...
    try:
        response = httpx.post(
            ...
            json={
                ...
                "to": [f"{name} <{email}>"],
                "sender": f"{settings.SMTP_FROM_NAME} <{settings.SMTP_FROM}>",
                "subject": "Recuperação de Senha — SGI Pro",
                "html_body": html,
            },
            timeout=10,
        )
        ...
        if response.status_code == 200 and data.get("data", {}).get("succeeded") == 1:
            return True
        print(f"[EMAIL ERROR] SMTP2GO response: {data}")
        return False
